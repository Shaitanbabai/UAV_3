...
test = [100, 250, 500, 250, 100]
total_dist = test[0]
for i in range(1, len(test)): # len(test) = 5 ### range(1, len(test)) = (1, 2, 3, 4)
    print(f"Итерация №{i}")
    print(f"total_dist = {total_dist}")
    cur_runner = test[i]
    print(f"cur_runner = {cur_runner}")
    last_runner = test[i-1]
    print(f"last_runner = {last_runner}")
    total_dist += abs(cur_runner - last_runner)
    print(f"Разница = {abs(cur_runner - last_runner)}\n\n")
print(total_dist)
